...
| F-37 | a validity rung is the wrong shape for the split | validity is an **ordinal in a 3-bit field** and `BLOCK_VALID_TREE`/`BLOCK_VALID_TRANSACTIONS` are **adjacent**, so a rung between them renumbers three persisted values and needs a migration — to express something that is not ordinal. Of what `BLOCK_VALID_TRANSACTIONS` certifies, a commitment block proves the structural half alone and leaves two body properties `ConnectBlock` re-checks anyway | `source` | stands |
| F-38 | `BLOCK_HAVE_BODIES = 256` costs **zero** relaxed assertions | the commitment block is stored like any block, so `BLOCK_HAVE_DATA` stays honest. Both 0.1 relaxations reverted; net change is one sibling tracker `pindexFirstMissingBodies`, **two added** assertions, and two guards narrowed — and the narrowing is inert on any state an unmodified node can reach, because bodies and data are recorded together | `bench-rt` | stands |
...
| F-40 | **the bit alone wedges the node permanently** | with `HAVE_DATA` honest, all four "do I need this block?" sites answer yes: `validation.cpp:AcceptBlock`'s `fAlreadyHave` and three in `net_processing.cpp`. Measured: withholding removed, **zero** requests, stuck at the fork forever. So the bit and the fetch layer are **one deliverable** — encoding (c) costs no relaxations and does not work at all until "have it" means "and its bodies" | `bench-rt` | stands |
| F-41 | the bit is persisted, so it needs a migration | a pre-bit datadir loads with `HAVE_BODIES` clear on every entry and trips the new "active chain ⇒ HAVE_BODIES" assertion at once. One upgrade pass at first load is the obvious fix and is **unwritten and untested** | `source` | **open** |
