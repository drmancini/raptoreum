...
| F-43 | **the rung sits after six checks, and they are not trivial** | of thirteen checks between `AcceptBlockHeader` and `ConnectBlock`, six are answerable from a commitment block alone: proof of work, **merkle root**, duplicate-transaction mutation, size, first-tx-is-coinbase, BIP34 height and the DIP3 coinbase type. A commitment block can honestly certify that it is well formed and commits to a specific non-duplicated identifier set at a height, with a valid coinbase (§2.4) | `source` | stands |
| F-44 | only **two** rules have no connect-time home | `ConnectBlock` re-invokes `CheckBlock`, so `bad-cb-multiple`, `CheckTransaction` and sigops are re-checked with bodies in hand. Not re-invoked is `ContextualCheckBlock`, leaving `nLockTime` finality (`bad-txns-nonfinal`) and type/version (`bad-txns-type`, `bad-txns-cb-type`). So R-28's problem is **two rules, not a category** — move them to connect, or re-invoke that function there | `source` | stands |
| F-45 | `fChecked` must not survive body assembly | `CheckBlock` early-returns on `block.fChecked` and sets it after a full pass, so `ConnectBlock`'s re-check is skipped for an object already checked in memory. A block assembled from commitments plus fetched bodies that inherits the flag from its commitment-only pass **never has its bodies checked at all**. One flag, and it is the difference between the design working and accepting anything | `source` | stands |
| F-46 | a third byte-indexed constant, same class as K-9 and F-30 | both size checks compare `block.vtx.size()` — a transaction **count** — against `MaxBlockSize()`, a byte limit. Harmless today; under the commitment format the count is what grows, so it silently becomes a real limit at a value nobody chose | `source` | stands |
