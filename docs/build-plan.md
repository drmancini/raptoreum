...
Nothing in phase 1 starts before **0.1c** settles the rung. 0.1 returned its verdict on 2026-09-18 — no kill — but it reached that verdict by withholding a block it had already validated (F-25e3), so the question that decides the *encoding* is still open, and it is a de-risking question, not a build one.
...
| 0.1c | **The rung probe — the question 0.1 did not reach, and the gate for phase 1.** A commitment-only block cannot honestly hold `BLOCK_VALID_TRANSACTIONS`, which `chain.h` defines as transactions valid, sigops, size and merkle root. In the commitment format that definition **splits cleanly**: the merkle root is over identifiers and *is* checkable from the commitment block alone, while transaction validity, sigops and size are not. So the new rung is not an invention — it is the half of `VALID_TRANSACTIONS` the commitments already prove. Settle which of two encodings the design takes, and prove it on the probe: **(a)** `nTx` from the commitment list with validity at the new rung, which makes the `nTx > 0 ⇒ VALID_TRANSACTIONS` assert in `validation.cpp:CheckBlockIndex` a third relaxation on the chain the kill criterion names; or **(b)** `nTx` left at 0 until bodies arrive, which costs nothing in chain selection — a commitment-only block is then simply not a candidate, which under bodies-required it could never be anyway — but gives up counting commitments and so cannot answer "how far behind am I in bodies". **Pass**: one encoding reaches all six scenarios with the assert expressed as a per-block state rather than a global flag (F-25k). **Fail**: both encodings need the named chain relaxed globally — then §15 becomes the design after all. | 1 w | **the open question** |
...
| 0.4 | **Smartnode swarm bring-up.** ProTx registrations, collateral, DKG and quorum formation on the 12-node WAN swarm — never attempted. Needs a test-only patch: `-llmqtestparams` overrides only size and threshold, leaving `dkgBadVotesThreshold`, `signingActiveQuorumCount`, `recoveryMembers` and `keepOldConnections` at three-member values. Turns every existing figure from an upper bound into a measurement. **And it is the only place to run the ChainLock-across-a-gap scenario 0.1 could not**: `EnforceBestChainLock` runs every scheduler tick and `assert(false)`s if `MarkConflictingBlock` fails, and with a commitment-only block on the chainlocked chain each tick re-inserts it as a candidate for `FindMostWorkChain` to evict again. | 2-3 w | fiddly, known |
