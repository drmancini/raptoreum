...
| 1.2 | **Block resource budget** (§1A). Sigop cap re-based to committed count and a consensus body-byte cap — the per-transaction work cap originally proposed here is **dropped** (D-17): it bounded nothing the sigop budget didn't already, and had no connect-time home without 1.3. **0.3 measured the unit: an accurate count over *spent* scripts, since legacy counting charges an 800-input bare-multisig spend 2 sigops for 2.35 s of work; the constant is ~130 µs per sigop, open to re-verification at a packed worst-case shape (F-91).** Pre-implementation review (2026-09-19) found and fixed in design: the counting rule is additive (keeps legacy's scriptSig/output term, extends spent-script counting past P2SH-only) not a replacement (F-86); must add `CHECKDATASIG` explicitly via a new path, never by editing the shared legacy counter (F-87); gated behind a boolean mirroring `fDIP0001Active`, test-only until 4.6 (F-88); miner and mempool accounting included, not deferred (F-89). See §1A's revision and R-32. | 2-3 w | design-sensitive — scoped, not yet built |
