...
| 1.2 | **Block resource budget** (§1A). Sigop cap re-based to committed count and a consensus body-byte cap — the per-transaction work cap originally proposed here is **dropped** (D-17): it bounded nothing the sigop budget didn't already, and had no connect-time home without 1.3. **The sigop budget is built and set to 250,000 (D-18, R-33)**: `GetAccurateSigOpCount` closes F-86 (additive, not a replacement — keeps legacy's scriptSig/output term, extends spent-script counting past P2SH-only) and F-87 (`CHECKDATASIG`, via a new opt-in path, never by editing the shared legacy counter); `g_commitmentBudgetActive` (test-only until 4.6) gates it per F-88, threaded through ATMP, `ConnectBlock`, both legacy checks and the miner (F-89) — one call site fixes the miner too, since it reads sigops straight from the mempool entry. Integration-tested end to end. **F-91 resolved (R-34): D-18 stands** — a live packed-shape benchmark found an `~O(n²)` input-count cost (F-93, up to ~370-385 ms per K-3-sized transaction) that dwarfs the sigop-cost model, but it lives in mempool ATMP, not `ConnectBlock` (F-94, ~70 ms for the same transaction, cold) — a separate, likely pre-existing mempool DoS question, not a 1.2 defect. **Still open: the body-byte cap's value.** | 2-3 w | design-sensitive — sigop budget built, body-byte cap not started |
