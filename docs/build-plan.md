...
| 1.3 | **Acceptance layer.** Having the identifiers and having the bodies become two facts: a new status bit (a persisted block-index format change), a new validity rung, and the sites above `ConnectBlock` that read "do we have this block" — chain selection, candidate maintenance, `CheckBlockIndex`'s invariants, `NewPoWValidBlock`, pruning, `VerifyDB`, crash replay, `-reindex`, and the `InvalidateBlock` / `MarkConflictingBlock` / `EnforceBestChainLock` interactions. Plus a durable retry queue with per-block exponential backoff that never keys off accumulated work. **Re-estimated after 0.1**, and only for the half the probe reached: chain selection is largely built already, for pruning (F-25f). Everything else stands — the rung (which decides whether the probe's encoding survives at all, F-25e3), the persisted bit, out-of-order arrival (F-25j), the fetch cursor at `net_processing.cpp:FindNextBlocksToDownload` that jumps the gap (F-25e2), the unrequested and compact-block arrival paths (F-36), `-reindex`, ChainLock enforcement, and equal-work ties (F-35). **Started 2026-09-19 — broken into 8 ordered sub-steps** (below), each its own TDD/mutation/Fable-review unit, sequenced to respect D-16's encoding decision and F-40's "bit and fetch are one deliverable" finding. | **8-16 w** | **partly known** — chain selection demonstrated; the rung, the bit and the fetch layer are not |
| 1.3.1 | **Replace the probe's global flag with the real per-block bit.** Add `BLOCK_HAVE_BODIES = 256` to `chain.h`'s `BlockStatus` (128 is taken by `BLOCK_CONFLICT_CHAINLOCK` — §2.1's proposed value). `ReceivedBlockTransactions` (`validation.cpp:3831-3847`) already splits commitment-level (`nTx`, `nChainTx`) from body-level (`nFile`, `nDataPos`, `BLOCK_HAVE_DATA`) — the change is mechanical there: `fHaveCommitmentOnly = true` becomes `pindexNew->nStatus |= BLOCK_HAVE_BODIES` on the `pos != nullptr` branch instead. The hard part is everywhere else: add `pindexFirstMissingBodies` beside `pindexFirstMissing` in `CheckBlockIndex` (F-38); one new assertion, `HAVE_BODIES ⇒ HAVE_DATA`; do **not** add "on active chain ⇒ bodies" (F-38b: false under pruning); add F-38c's recovery rule (above a body gap, outranking tip, not a candidate ⇒ must be in `m_blocks_unlinked`); narrow the two guards at `validation.cpp:5566` and `:5645` from the global `fHaveCommitmentOnly` to the real per-block bit; add a per-block `IsCommitmentOnly()`-equivalent accessor rather than extending `IsBlockPruned()` (F-25h — that extension was already tried and reverted, since `GetBlockChecked`'s existing "not found on disk" answer is already truthful and nothing decides differently on the extended one). F-41's migration (a pre-bit datadir loads with `HAVE_BODIES` clear on every entry, which then stops serving all history and breaks `VerifyDB`'s first iteration) needs a one-time upgrade pass at load: `HAVE_DATA` set on a pre-1.3 record implies bodies were always inline, so set `HAVE_BODIES` too. | 1-2 w | mechanical split, wide blast radius |
| 1.3.2 | **The rung, per D-16: no new ordinal.** Split `CheckBlock`/`ContextualCheckBlock` (§2.4's table) into the ~9 commitment-checkable rows (PoW, header chain, merkle root, merkle malleation, size, coinbase-is-first, coinbase `CheckTransaction` including the founder payment, coinbase finality, `bad-cb-type`) and the 3 body-only rows with no connect-time home (F-44): `nLockTime` finality, type/version (`bad-txns-type`/`bad-cb-type`), and `bad-txns-oversize` (F-44b — the one that matters most, since it's §1A's own multiplier and K-6's quadratic-sighash bound). `BLOCK_VALID_TRANSACTIONS` is granted at accept time on the commitment rows alone; the 3 body-only rows relocate to run at connect time (`ConnectBlock` already re-invokes `CheckBlock` but not `ContextualCheckBlock` — either re-invoke it there or move just these 3 checks into `ConnectBlock`'s own body). Add the two rules the format needs and nothing today checks (F-43b): identifier uniqueness (trivial set check over the list) and `id[0] == hash(coinbase)`. Close F-45's `fChecked` hazard (a height-dependent verdict — the founder payment — cached across a materialisation that must not reuse the old object): confirm whatever assembles a full block from commitments + fetched bodies follows `PartiallyDownloadedBlock::FillBlock`'s existing rule of producing a fresh `CBlock`, never mutating a cached one in place. | 2-3 w | the rung's own correctness is the fork's whole safety argument |
| 1.3.3 | **Fix the two "already have it" blind spots the bit alone would wedge on (F-40).** `net_processing.cpp:810-811`'s `FindNextBlocksToDownload` cursor advances `pindexLastCommonBlock` on `HaveTxsDownloaded()` (count-based, true for a commitment-only block) — must test the real bodies-bit instead, or the cursor jumps the gap and never returns (F-25e2, observed: re-requested twice, then dropped). Two more `nTx != 0` sites read "already had it" the same wrong way (F-36): `AcceptBlock`'s unrequested-block drop, and `FindNextBlocksToDownload`'s compact-block-relay fallback — both must check the bodies-bit before discarding a compact-block-relayed body as redundant. F-40b's deeper gap: the probe's fix only converged via **restart** (which resets each peer's `pindexLastCommonBlock`); a live two-peer harness is required to prove the cursor fix alone converges without one, since a fresh peer bought only one more request before going flat. Deliberately minimal — enough not to wedge, not phase 2.2's full position-addressed multi-source protocol. | 2-3 w | the one component with no working live-peer proof yet |
| 1.3.4 | **`CheckBlockIndex` invariants for the real (per-block) encoding.** Generalise F-25g's two existing relaxations off the global flag. Reach and handle F-25j's predicted-but-unreached assert: a commitment-only block whose **parent's** commitments haven't arrived yet lands in `m_blocks_unlinked` with no `HAVE_DATA` at all, tripping an assertion a single-peer in-order harness cannot produce — needs a genuinely multi-peer, out-of-order test harness, since the design fetches from many peers and this is normal there, not an edge case. | 1 w | needs new test infrastructure (multi-peer, out-of-order) |
| 1.3.5 | **Equal-work tie-break — resolve F-42, don't leave it a hypothesis.** Re-stamping `nSequenceId` (`validation.cpp`'s `ReceivedBlockTransactions` descendant walk) when bodies arrive after commitments already did once re-stamps the whole subtree, changing which side of an equal-work tie wins vs. unmodified behaviour (F-35). Decide whether to re-stamp on the bodies-arrived event, and prove the chosen behaviour with a test — not "hypothesis, not run." | 3-5 d | small scope, easy to skip by accident |
| 1.3.6 | **`VerifyDB` / crash replay / `-reindex`.** `CValidationState::BodiesMissing()` (`validation.cpp:3139`, four call sites, already built during the 0.1 probe) turns "corrupted, please -reindex" into an honest "bodies missing, not corrupt" (F-25/F-25c) — re-verify it against the real per-block bit from 1.3.1, not the probe's global flag, and extend to `-reindex`/`-reindex-chainstate`, which already skip `ContextualCheckBlock` today for an unrelated pre-existing reason (§2.4). | 3-5 d | mostly re-pointing existing probe logic at the real bit |
| 1.3.7 | **Durable retry queue, per-block exponential backoff, never keyed on accumulated work.** `m_blocks_unlinked` already re-arms retry for free (F-25f), but on the ordinary block-request cadence — no backoff, and today's backoff heuristics (where they exist elsewhere) key on a chain's accumulated work, which is exactly wrong for a body gap that isn't on the current best-work chain. New: durable (survives restart), backs off per missing body independent of the block's own chain-work standing, and — §2.3's inviolable rule — never marks the block invalid regardless of how long the wait runs. | 1 w | new component, no existing analogue to extend |
| 1.3.8 | **`InvalidateBlock` / `MarkConflictingBlock` / `EnforceBestChainLock` — audit, not full hardening.** Confirm these three don't crash or silently corrupt state when invoked against a commitment-only block. Full graceful degradation (`EnforceBestChainLock`'s `assert(false)` made non-fatal, etc.) is 4.1's job — 1.3 only needs to prove these three are safe under the new state, not polished. | 3-5 d | narrow, defers the real fix to 4.1 |
