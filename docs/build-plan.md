...
Nothing in phase 1 starts before **0.1c** settles the rung. **Partly settled 2026-09-18** — the bit is settled and is inseparable from the fetch layer (F-38, F-40); the rung is reopened (R-28, F-37b). 0.1 returned its verdict on 2026-09-18 — no kill — but it reached that verdict by withholding a block it had already validated (F-25e3), so the question that decides the *encoding* is still open, and it is a de-risking question, not a build one.
...
| 0.1c | **The rung probe. PARTLY DONE 2026-09-18 — the bit is settled, the rung is not.** Settled: `BLOCK_HAVE_BODIES = 256` with the commitment block stored normally costs **zero relaxed assertions** (F-38), one added assertion plus one recovery rule, and it holds through arrival, restart and a pruned node at height 1,200. Also settled: the bit **cannot ship without the fetch layer** — every "do I have this block?" answers yes and the node never asks again (F-40). **Retracted** (R-28): "no rung is needed". `ConnectBlock` does not re-invoke `ContextualCheckBlock`, so `nLockTime` finality, DIP3 coinbase type, `bad-txns-type`, BIP34 and oversize are enforced only at accept (F-37b) — a body-less block cannot honestly hold `VALID_TRANSACTIONS`. **Remaining**: choose the rung encoding knowing it costs a persisted renumber (F-37), and build a harness that can withhold a body the node has *never validated* — the current one validates then flips a bit, and that has produced a wrong answer twice. | +1 w | **rung still open** |
