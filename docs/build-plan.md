...
**The one genuine unknown, now priced.** The acceptance layer. No production chain has a block
whose acceptance depends on a network fetch, so there was nothing to copy and phase 0 existed to
price it. **0.1 has run (2026-09-18): no kill.** The half that looked hardest — teaching chain
selection to hold a block it cannot complete, without condemning it, and to pick it up when the
rest arrives — turned out to be already built, because pruning needs the same thing and
`FindMostWorkChain` already declines absent-data chains and re-arms them through
`m_blocks_unlinked` (F-25f). What is genuinely new is narrower than the phase assumed: the body
store, the fetch scheduler, and a persisted bit that separates "have the commitments" from "have
the bodies" (F-25i).
...
|---|---|---|---|
| 0.1 | **Acceptance probe. DONE 2026-09-18 — verdict: no kill, and 1.3 is smaller than it was priced.** Phase A measured the blast radius of a missing body: `VerifyDB` called it database corruption and refused to start, P2P serving aborted the process, `ConnectTip` hit `AbortNode`, RPC degraded gracefully. Phase B gated all three on a predicate and then moved the withholding from read time to **accept** time, which is the only way to reach the states that matter (F-25d). Result: the hold-and-retry behaviour §2.3 wants already exists for pruning (F-25f); `ReceivedBlockTransactions` is the whole layer in one function and splits cleanly (F-25e); the total cost was that split plus **two** `CheckBlockIndex` relaxations in the direction pruning already established (F-25g). Four scenarios pass: arrival, restart while incomplete, re-arm on arrival, and a real body gap under held blocks (F-25h). `DisconnectTip` across a gap — the path flagged as likeliest to fire the criterion — **cannot arise** without dual validation (F-29). No consensus check was touched. | 3-5 d | **done** |
...
| 1.3 | **Acceptance layer.** Having the identifiers and having the bodies become two facts: a new status bit (a persisted block-index format change), a new validity rung, and the sites above `ConnectBlock` that read "do we have this block" — chain selection, candidate maintenance, `CheckBlockIndex`'s invariants, `NewPoWValidBlock`, pruning, `VerifyDB`, crash replay, `-reindex`, and the `InvalidateBlock` / `MarkConflictingBlock` / `EnforceBestChainLock` interactions. Plus a durable retry queue with per-block exponential backoff that never keys off accumulated work. **Re-estimated after 0.1**: the probe held a commitment-only block, restarted on it and re-armed it with one function split and two debug-invariant relaxations, because the chain-selection half is already built for pruning (F-25e, F-25f, F-25g). What remains is the body store, the fetch scheduler and the separate bit the probe did without — real work, but no longer unknown work. | **6-10 w** | **mostly known** — the state and its chain-selection handling are demonstrated; the fetch scheduler and the persisted bit are not |
