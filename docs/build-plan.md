...
Nothing in phase 1 starts before **0.1c** settles the rung. **Settled 2026-09-18** — no rung, one bit, and the bit is inseparable from the fetch layer (F-37, F-38, F-40). 0.1 returned its verdict on 2026-09-18 — no kill — but it reached that verdict by withholding a block it had already validated (F-25e3), so the question that decides the *encoding* is still open, and it is a de-risking question, not a build one.
...
| 0.1c | **The rung probe. DONE 2026-09-18 — answer: there is no rung, there is a bit and a fetch.** A rung is the wrong shape: validity is a 3-bit ordinal and `TREE`/`TRANSACTIONS` are adjacent, so inserting one renumbers three persisted values (F-37). `BLOCK_HAVE_BODIES = 256` with the commitment block stored normally costs **zero relaxed assertions** — two added, two guards narrowed inertly (F-38). But the bit alone **wedges the node permanently**: every "do I need this block?" answers yes, and the node made zero requests for missing bodies (F-40). Arrival, restart and convergence all pass once the fetch is body-aware. **Not run**: out-of-order arrival, competing tip, reorg onto a gapped branch, ChainLock across a gap, `-reindex`, and the migration the bit implies (F-41). | 1 w | **done, partly** |
